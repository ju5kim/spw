...
<%
	request.setAttribute("siteTitle", "판다 목록");
%>
...
<script type="text/javascript">
	...
	/* 	listForm.action="insertForm.spw";
		listForm.submit(); */
		location.href="generalInsertForm.spw?gsort=2";
	}
	...
	/* 상세보기 : generalDetailForm */
 	function generalDetailForm(){
		...
		$("#pList #gnum").val(gnum);
		$("#pList").attr("action", "generalDetailForm.spw");
		$("#pList").submit();
	} 
 	/* 날짜검색 */
	function searchDate(){
		...
		function whenSuccess(data){
			console.log(" ajax 성공 !!!! ");
			//console.log(data);
		    $("#pList div.list_grid").replaceWith($(data).find("#pList div.list_grid"));
		    $("#pList div.list_paging paging_saja").replaceWith($(data).find("#pList div.list_paging paging_saja"));
		}
		//실패했을 때
		function whenError(){
			console.log("데이터 없음 !! ");
	...
	<div class="list_contents">
		<div class="list_top">
			...
			<div class="search_by_date">
				...
				<input type="button" value="조회" id="btnSerachByDate" name="btnSerachByDate" onclick="searchDate()" class="sitebtn">
			</div>
			<br>
			<div class="" align="right">
				<input type="button" value="상품등록" class="sitebtn" onclick="generalInsertForm()" />
			</div>
			<form id="pList" name="pList" method="POST">
<%
