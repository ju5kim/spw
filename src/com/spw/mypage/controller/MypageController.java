package com.spw.mypage.controller;
...
import com.spw.mypage.service.MypageService;
 ...
 * 클래스
 * 설명
 * : 마이페이지 컨트롤러.(회원 수정, 찜하기, 알림, 사자,판다,경매 목록)
 * 날짜 : 
 * 1차 - 2020.07.30(목)
 * 작성자 : 장효제.
 */
...
public class MypageController {
	...
	public ModelAndView memberInfo(@ModelAttribute MemberVO mvo) {
	...
	public ModelAndView myGeneral(@ModelAttribute GeneralVO mgvo, HttpServletRequest req) {
		...
		// 사자, 판다 조회 시 버튼에서 넘어오는 구분. (조회시 구분이랑 회원번호필요.)
		String gsort = req.getParameter("gsort");
		// 사자, 판다 조회 시 로그인된 세션의 회원번호 가져오기.
		HttpSession hs = req.getSession();
		String mnum = (String)hs.getAttribute("mnum");
		// 구분과 회원번호 vo 에 지정.
		log.info("세션 안 회원번호 : "+mnum);
	...
	public ModelAndView myGeneralDel(@ModelAttribute GeneralVO mgvo, HttpServletRequest req) {
		...
		// 사자, 판다 삭제시 버튼에서 넘어오는 구분. (삭제시 구분이랑 회원번호필요.)
		String gsort = req.getParameter("gsort");
		// 사자, 판다 삭제시 로그인된 세션의 회원번호 가져오기.
		HttpSession hs = req.getSession();
		String mnum = (String)hs.getAttribute("mnum");
		// 구분과 회원번호 vo 에 지정.
		log.info("세션 안 회원번호 : "+mnum);
